...
function App() {
  ...
  useEffect(() => {
    if (useSearchParams.size > 0) {
      showPayment = true;
      let upiData = upiJson.filter((item) => (item.id == useSearchParams.get('qr')))[0];
      if (upiData) { 
        setUpi(upiData.upi_id ?? '');
        setMarchant(upiData.upi_id  ?? '');
        setName(upiData.merchant_name ?? '');
          ...
          <Row style={{ maxWidth: '950px', width: '100%' }}>
            <Col md={{ span: 6 }} className={showPayment ? 'd-none' : ''}>
              ...
              <Form.Group className="mb-2" controlId="merchantName">
                ...
                <Form.Control type="text" name='name' value={name} placeholder="Enter your business name here..." onChange={handleUpiId} />
              </Form.Group>
              <Form.Group className="mb-2" controlId="upiId">
                <Form.Label>UPI ID</Form.Label>
                <Form.Control type="text" name='upi' value={upi} placeholder="Enter your UPI ID here..." onChange={handleUpiId} />
              </Form.Group>
              <Form.Group className="mb-2" controlId="transactionAmount">
                <Form.Label>Transaction Amount</Form.Label>
                <Form.Control type="text" name='amount' value={amount} placeholder="Enter transaction amount here..." onChange={handleUpiId} />
              </Form.Group>
              <Form.Group className="mb-2" controlId="transactionDescription">
                <Form.Label>Description (Notes)</Form.Label>
                <Form.Control type="text" name='description' value={description} placeholder="Enter description here..." onChange={handleUpiId} />
              </Form.Group>
            ...
            <Col md={{ span: 6 }} className={showPayment ? 'd-none' : ''}>
              <div className='rounded-4 shadow border pb-3 bg-white' ref={contentRef}>
                <div className="mt-3">
                  <p className='text-center fw-bolder text-dark fs-4 text-uppercase lh-md py-1 bg-light'>{name === '' ? 'Merchannt Name' : name}</p>
                </div>
                <div className='mt-4 d-flex justify-content-center'>
                  <QRCode value={qr} title={qr} size={225} level='L' />
                </div>
            ...
            <Col md={{ span: 6, offset: 3 }} className={showPayment ? '' : 'd-none'}>
              ...
              <Form.Group className="mb-2" controlId="button">
                ...
                {/* <Button variant="primary" className='w-100' onClick={handleDownload}>Download QR Code</Button> */}
                {/* <a href={qr}>Open Link On Phone</a> <span className='text-muted'>(Requires UPI App)</span> */}
                {/* <a href="#" onClick={() => handleCopy('text')}><img src={copyImage} alt="" srcSet="" /></a> */}
                {/* <a href="#" onClick={() => handleCopy('link')}><img src={linkImage} alt="" srcSet="" /></a> */}
              </Form.Group>
            </Col>
          </Row>
        </div>
